...
The Android-first native Member client lives in `mobile/` as a standalone bare React Native project. It shares this repository and release version with the Go binaries but has its own npm lockfile and Gradle build; no monorepo framework is used.

Install Node.js 22.11+, JDK 17+, and Android SDK 36, then run:
...
Attachment uploads default to 10 MiB per file and 1 GiB total. The Community Owner can change the per-file limit up to 256 MiB at runtime at `/admin/settings`; `ALLCHAT_MAX_ATTACHMENT_BYTES` supplies the initial value for a new Instance. Operators may configure total storage with `ALLCHAT_MAX_ATTACHMENT_STORAGE_BYTES`. All values remain bounded by the built-in hard ceilings.
