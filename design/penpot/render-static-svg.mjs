...
try{const page=await browser.newPage({viewport:{width:1280,height:800}});
 ...
 },frame);const size=await page.evaluate(()=>{const s=document.querySelector('svg');return {width:Math.ceil(Number(s.getAttribute('width'))),height:Math.ceil(Number(s.getAttribute('height')))}});await page.setViewportSize(size);await page.evaluate(()=>document.fonts.ready);frame.png=frame.filename.replace(/\.svg$/,'.png');await page.screenshot({path:directory+'/'+frame.png});}
 for(let offset=0;offset<frames.length;offset+=4){let html='<style>body{margin:0;background:#fff;font:14px sans-serif}.grid{display:grid;grid-template-columns:640px 640px}figure{margin:0}figcaption{height:24px;padding:3px 8px;box-sizing:border-box}img{width:640px;height:400px;object-fit:contain;display:block}</style><div class="grid">';for(const frame of frames.slice(offset,offset+4)){const data=(await fs.readFile(directory+'/'+frame.png)).toString('base64');html+=`<figure><figcaption>${frame.name}</figcaption><img src="data:image/png;base64,${data}"></figure>`;}html+='</div>';await page.setViewportSize({width:1280,height:848});await page.setContent(html);await page.screenshot({path:directory+'/contact-'+(offset/4+1)+'.png'});}
