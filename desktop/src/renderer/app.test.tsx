...
describe('desktop renderer bootstrap', () => {
  ...
  it('renders authenticated Community navigation from the versioned bootstrap contract', async () => {
    ...
    const executeInstance = vi.fn(async (_instanceId: string, action: { type: string }) => {
      ...
      if (action.type === 'get_community_settings') return { type: 'community_settings' as const, settings: { max_attachment_mib: 64, home_markdown: '# Welcome', push_relay_url: 'https://push.example.com', push_key_id: 'key-1', push_public_key: 'public-key' } };
