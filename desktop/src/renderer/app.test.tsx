...
describe('desktop renderer bootstrap', () => {
  ...
  it('renders authenticated Community navigation from the versioned bootstrap contract', async () => {
    ...
    expect(screen.getByRole('navigation', { name: 'Community settings' })).toBeVisible();
    ...
    await waitFor(() => expect(within(communityAdministration).getByRole('heading', { name: 'Community ringtone' })).toBeVisible());
    expect(within(communityAdministration).getByRole('heading', { name: 'General' })).toBeVisible();
    ...
    fireEvent.change(lobbyComposer, { target: { value: 'Unsent draft' } });
    fireEvent.click(screen.getByRole('button', { name: 'Reply' }));
    expect(screen.getByText('Replying to nora')).toBeVisible();
    expect(lobbyComposer).toHaveFocus();
    expect(document.getElementById('message-message-1')).toHaveClass('message-reply-target');
    expect(document.querySelector('.reply-composer-preview')).toHaveTextContent('Desktop parity starts here');
    fireEvent.click(screen.getByRole('button', { name: 'Cancel reply' }));
    expect(lobbyComposer).toHaveValue('Unsent draft');
    expect(lobbyComposer).toHaveFocus();
    expect(document.querySelector('.message-reply-target')).toBeNull();
    fireEvent.click(screen.getByRole('button', { name: 'Reply' }));
    fireEvent.keyDown(lobbyComposer, { key: 'Escape' });
    expect(screen.queryByRole('button', { name: 'Cancel reply' })).not.toBeInTheDocument();
    expect(lobbyComposer).toHaveValue('Unsent draft');
    fireEvent.click(screen.getByRole('button', { name: 'Edit' }));
    expect(screen.getByRole('form', { name: 'Edit message' })).toBeVisible();
    expect(lobbyComposer).toHaveValue('Unsent draft');
    expect(screen.getByLabelText('Edit message text').closest('.message')).not.toBeNull();
    expect(screen.getByRole('img', { name: 'landscape.png' })).toBeVisible();
    fireEvent.click(screen.getByRole('button', { name: 'cancel' }));
    expect(lobbyComposer).toHaveValue('Unsent draft');
    fireEvent.change(lobbyComposer, { target: { value: '' } });
    ...
    expect((editor as HTMLTextAreaElement).value).toContain('Desktop parity starts here');
    expect(lobbyComposer).toHaveValue('');
    ...
    expect(within(voiceSettings).getByLabelText('Camera')).not.toBeVisible();
    ...
    fireEvent.click(within(voiceSettings).getByRole('button', { name: 'Camera & sharing' }));
    expect(within(voiceSettings).getByLabelText('Camera')).toBeVisible();
    fireEvent.click(within(voiceSettings).getByRole('button', { name: 'Reset settings' }));
    ...
    expect(memberPopover).toHaveStyle({ position: 'fixed', left: '472px', top: '240px' });
    ...
    expect(within(directCallGrid).getByText('You')).toBeVisible();
		expect(within(screen.getByRole('region', {name: 'Call controls'})).getByRole('button', { name: 'Open Activities' })).toBeVisible();
    ...
    fireEvent.click(within(voiceMenu).getByRole('menuitem', { name: 'View profile' }));
    ...
    await waitFor(() => expect(screen.queryByRole('region', { name: 'Voice controls' })).not.toBeInTheDocument());
  }, 15_000);
});
