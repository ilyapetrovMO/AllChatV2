...
describe('desktop shared control styling', () => {
  ...
  it('reserves space for the trailing Search icon after global input rules', () => {
    expect(css).toMatch(/\.header-search input\s*\{[^}]*padding:\s*5px 36px 5px 9px;/s);
    expect(css).toMatch(/\.header-search > \.lucide-icon\s*\{[^}]*right:\s*10px;/s);
    expect(css.indexOf('.header-search input { box-sizing: border-box;')).toBeGreaterThan(css.indexOf(':where(input:not('));
  });
