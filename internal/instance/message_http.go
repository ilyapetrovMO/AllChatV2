...
document.getElementById("cancel-edit").addEventListener("click",stopEditing);bodyInput.addEventListener("keydown",event=>{if(event.defaultPrevented)return;if(event.key==="ArrowUp"&&!bodyInput.value&&!composer.dataset.editing&&!composer.dataset.replying&&mentionMenu.hidden){const latest=[...messages.querySelectorAll("[data-edit-message]")].at(-1);if(latest){event.preventDefault();beginEditing(latest)}}else if(event.key==="Escape"&&composer.dataset.editing){event.preventDefault();stopEditing()}else if(event.key==="Escape"&&composer.dataset.replying){event.preventDefault();stopReply()}else if(event.key==="Enter"&&!event.shiftKey&&!event.isComposing){event.preventDefault();composer.requestSubmit()}});
