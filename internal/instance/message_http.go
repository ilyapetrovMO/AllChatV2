...
func imageSource(contentType, previewURL, originalURL string) string {
	if strings.EqualFold(contentType, "image/gif") || previewURL == "" {
		return originalURL
	}
	return previewURL
}
...
var channelTemplate = template.Must(template.New("channel").Funcs(template.FuncMap{
	"messageJSON": func(value any) string { data, _ := json.Marshal(value); return string(data) },
...
function attachmentElement(attachment){const contentType=attachment.content_type||"";if(contentType.startsWith("image/")){const button=document.createElement("button"),image=document.createElement("img");button.type="button";button.className="message-image-button";button.dataset.originalSrc=attachment.url;button.dataset.imageName=attachment.name;button.dataset.imageSize=String(attachment.size||0);button.setAttribute("aria-label","Open "+attachment.name);image.className="message-image";image.src=contentType.toLowerCase()==="image/gif"?attachment.url:attachment.preview_url||attachment.url+"/preview";image.alt=attachment.name;image.loading="lazy";button.append(image);return button}if(contentType.startsWith("video/")||contentType.startsWith("audio/")){const media=document.createElement(contentType.startsWith("video/")?"video":"audio"),source=document.createElement("source"),fallback=document.createElement("a");media.className=contentType.startsWith("video/")?"message-video":"message-audio";media.controls=true;media.preload="metadata";source.src=attachment.url;source.type=contentType;fallback.href=attachment.url;fallback.textContent="Download "+attachment.name;media.append(source,fallback);return media}const link=document.createElement("a");link.className="message-attachment";link.href=attachment.url;link.append(window.allchatIcon("paperclip"),document.createTextNode(attachment.name));return link}
...
const composer=document.getElementById("composer"),bodyInput=document.getElementById("message-body"),fileInput=document.getElementById("attachment"),editingBanner=document.getElementById("editing-banner");
...
bodyInput.required=false;const attachmentLabel=fileInput.previousElementSibling,attachmentPreviews=document.getElementById("attachment-previews");attachmentLabel.className="attachment-button";attachmentLabel.title="Add attachments";attachmentLabel.setAttribute("aria-label","Add attachments");window.allchatSetIcon?.(attachmentLabel,"paperclip");let selectedFiles=[],attachmentPreviewURLs=[];
...
function stopEditing(){composer.action=composer.dataset.publishAction;delete composer.dataset.editing;bodyInput.value="";bodyInput.placeholder="Message #{{.Channel.Name}}";fileInput.disabled=false;editingBanner.hidden=true;bodyInput.focus()}
function beginEditing(button){button.closest(".message")?.scrollIntoView({block:"center"});window.AllChatDesign?.editMessage(button)}
