...
window.renderAllChatMessage=render;document.querySelectorAll("#messages > .message").forEach(item=>{hydrateLinkPreview(item,item.querySelector(".body")?.textContent||"");const stamp=item.querySelector(".message-time");if(stamp)stamp.textContent=formatTimestamp(stamp.dataset.createdAt)});
let historyBefore={{.FirstSequence}},loadingHistory=false,historyExhausted=historyBefore<=1;
async function loadOlderMessages(){if(loadingHistory||historyExhausted)return;loadingHistory=true;const anchor=messages.querySelector(":scope > .message"),height=messages.scrollHeight;try{const response=await fetch("/api/v1/channels/"+channel+"/messages?before="+historyBefore+"&limit=50");if(!response.ok)throw new Error();const result=await response.json(),page=result.messages||[];if(!page.length){historyExhausted=true;return}for(const message of page){render(message,"message.created");const item=document.getElementById("message-"+message.id);if(anchor&&item)messages.insertBefore(item,anchor)}historyBefore=result.next_before||page[0].sequence;historyExhausted=!result.has_more;messages.scrollTop+=messages.scrollHeight-height}finally{loadingHistory=false}}
function trimHistory(){if(!conversationFollower.nearBottom())return;const items=messages.querySelectorAll(":scope > .message");for(let index=0;index<items.length-100;index++)items[index].remove();const first=messages.querySelector(":scope > .message");if(first?.dataset.sequence){historyBefore=Number(first.dataset.sequence);historyExhausted=false}}
messages.addEventListener("scroll",()=>{if(messages.scrollTop<80)loadOlderMessages();if(conversationFollower.nearBottom())trimHistory()},{passive:true});
...
socket.onmessage=event=>{const frame=JSON.parse(event.data);if(frame.type==="events"&&Array.isArray(frame.events)){frame.events.forEach(item=>socket.onmessage({data:JSON.stringify({type:item.type,cursor:item.cursor,channel_id:item.channel_id,payload:item.payload})}));if(frame.cursor>=cursor){cursor=frame.cursor;localStorage.setItem(key,String(cursor))}return}if(frame.type==="snapshot_required"){localStorage.setItem(key,String(frame.cursor));location.reload();return}if(frame.type==="channel.removed"&&frame.channel_id===channel){location.href="/";return}if(frame.type==="state.ephemeral"){document.getElementById("typing").textContent=typingSummary(frame.payload.typing);const indicator=document.getElementById("member-presence"),dnd=frame.payload.presence[member]==="dnd";indicator.classList.toggle("dnd",dnd);indicator.classList.toggle("online",!dnd);document.querySelectorAll("[data-participant-id]").forEach(item=>{const state=frame.payload.presence[item.dataset.participantId]||"offline",dot=item.querySelector(".participant-presence");if(!dot)return;dot.classList.toggle("online",state==="online");dot.classList.toggle("dnd",state==="dnd");dot.classList.toggle("offline",state!=="online"&&state!=="dnd")})}if(frame.payload)render(frame.payload,frame.type);if(frame.cursor>=cursor){cursor=frame.cursor;localStorage.setItem(key,String(cursor))}};
