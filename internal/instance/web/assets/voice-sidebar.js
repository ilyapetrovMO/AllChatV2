(() => {
  ...
  const renderStage = async () => {
    ...
    const participantIDs=new Set((state.participants||[]).map(participant=>participant.member_id));for(const video of session.remoteVideos?.values()||[]){if(video.dataset.stopped==="true")continue;const memberID=video.dataset.memberId||"";if(participantIDs.has(memberID))screensByMember.set(memberID,video);else unmatchedScreens.push(video)}
    for(const participant of state.participants||[]){const key=`participant:${participant.member_id}`,profile=state.members?.[participant.member_id]||{},screenStopped=session.stoppedVideoMembers.has(participant.member_id),screen=screensByMember.get(participant.member_id)||(!screenStopped&&participant.screen_sharing?unmatchedScreens.shift():null);let tile=existing.get(key);if(!tile){tile=document.createElement("article");tile.className="media-stage-tile participant-tile";tile.dataset.stageKey=key;const visual=document.createElement("div");visual.className="media-stage-visual";tile.append(visual,document.createElement("strong"))}const visual=tile.querySelector(".media-stage-visual"),sharing=!!screen;tile.classList.toggle("speaking",!!participant.speaking);tile.classList.toggle("sharing",sharing);tile.onclick=()=>{tile.classList.toggle("expanded");if(sharing)session.connection?.send("screen-quality",{owner_id:participant.member_id,quality:tile.classList.contains("expanded")?"high":"medium"})};tile.oncontextmenu=participant.member_id===session.profile.id?null:event=>{event.preventDefault();window.AllChatVoiceSettings.openParticipantVolumeMenu({memberID:participant.member_id,label:profile.display_name||profile.username||state.names?.[participant.member_id]||"Member",x:event.clientX,y:event.clientY})};if(sharing){if(screen.parentElement!==visual)visual.replaceChildren(screen)}else{let avatar=visual.firstElementChild;if(!avatar||avatar.matches("video")){avatar=document.createElement(profile.avatar_url?"img":"span");visual.replaceChildren(avatar)}const wantsImage=!!profile.avatar_url;if(wantsImage!==avatar.matches("img")){const replacement=document.createElement(wantsImage?"img":"span");avatar.replaceWith(replacement);avatar=replacement}if(wantsImage){avatar.className="";avatar.src=profile.avatar_url;avatar.alt=""}else{avatar.className="media-stage-avatar-fallback";avatar.textContent=Array.from(profile.display_name||profile.username||"?")[0].toUpperCase()}}tile.querySelector("strong").textContent=participant.member_id===session.profile.id?"You":profile.display_name||profile.username||state.names?.[participant.member_id]||"Member";let badge=tile.querySelector(".screen-sharing-badge"),reportedSharing=!screenStopped&&participant.screen_sharing;if((sharing||reportedSharing)&&!badge){badge=document.createElement("span");badge.className="screen-sharing-badge";badge.textContent="Sharing screen";tile.append(badge)}else if(!sharing&&!reportedSharing){badge?.remove()}desired.push(tile)}
  ...
  const makePanel = (roomID, name) => {
    ...
    window.allchatSetIcon?.(panel.querySelector("[data-voice-soundboard]"), "music");window.allchatSetIcon?.(panel.querySelector("[data-voice-screen]"), "monitor");window.allchatSetIcon?.(panel.querySelector("[data-voice-leave]"), "phone");
    ...
    try {
      ...
      const receiveTrack = event => {
        if (event.track.kind === "video") {
          ...
          const memberID=window.allchatMediaOwnerID?.(event.track.id,event.streams[0]?.id)||"";
        ...
        if (!audio) {
          const memberID=window.allchatMediaOwnerID?.(event.track.id,event.streams[0]?.id)||"";audio = document.createElement("audio"); audio.autoplay = true;audio.dataset.memberId=memberID;window.AllChatVoiceSettings.applyOutput(audio,memberID);
      ...
      const receiveFrame = frame => {
        if ((frame.type === "screen-low" || frame.type === "screen-medium" || frame.type === "screen-high") && session.screenSender) {
		  const parameters=session.screenSender.getParameters(),maximum=frame.type==="screen-low"?0:frame.type==="screen-medium"?1:2;(parameters.encodings||[]).forEach((encoding,index)=>encoding.active=index<=maximum);session.screenSender.setParameters(parameters).catch(()=>{});
          ...
          session.stoppedVideoMembers.add(frame.member_id);
          ...
          session.stoppedVideoMembers.delete(frame.member_id);
  ...
  addEventListener("allchat:voice-settings",()=>active?.remoteAudios?.forEach(audio=>window.AllChatVoiceSettings.applyOutput(audio,audio.dataset.memberId||"")));
  ...
  document.addEventListener("visibilitychange",()=>{active?.connection?.send("screen-visibility",{visible:!document.hidden});if(document.hidden)for(const video of active?.remoteVideos?.values?.()||[])active.connection?.send("screen-quality",{owner_id:video.dataset.memberId,quality:"low"})});
