...
export class MediaSession {
  ...
  async setCamera(enabled: boolean): Promise<void> {
    ...
    if (enabled && !existing) { const camera = await (this.options.getUserMedia || mediaDevices.getUserMedia)({audio: false, video: {facingMode: 'user'}}) as MediaStream; const track = camera.getVideoTracks()[0]; if (track) { this.local.addTrack(track); this.peer.addTrack(track, this.local); await this.renegotiate(); } }
  }
  ...
  async setScreenSharing(enabled: boolean): Promise<void> {
    ...
    stream.getTracks().forEach(track => this.peer?.addTrack(track, stream)); this.send({type: 'screen-visibility', visible: true});
    const videoTrack = stream.getVideoTracks()[0]; if (videoTrack) (videoTrack as unknown as {onended?: () => void}).onended = () => { if (this.screen === stream) this.setScreenSharing(false).catch(() => {}); };
  ...
  private async connect(takeover: boolean): Promise<void> {
    ...
    peer.onicecandidate = (event: {candidate?: {toJSON(): object}}) => { if (event.candidate) this.send({type: 'candidate', candidate: event.candidate.toJSON()}); };
