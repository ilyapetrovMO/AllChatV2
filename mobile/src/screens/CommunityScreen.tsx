...
export function mergeMessagePage(current: Message[], page: Message[]) {
  return [
    ...new Map([...page, ...current].map(item => [item.id, item])).values(),
  ].sort((a, b) => a.sequence - b.sequence);
}

export function trimMessageWindow(messages: Message[], limit = 100) {
...
export function CommunityScreen({
  ...
  const [history, setHistory] = useState<
    Record<string, { hasMore: boolean; nextBefore: number; loading: boolean }>
  >({});
  const realtime = useRef<RealtimeClient | null>(null);
  const client = useMemo(
    () => new AllChatClient(account.instance_url),
  ...
  const voiceChannelIDs = useMemo(
    () =>
      community?.channels
        .filter(item => item.type === 'voice' && !item.archived)
        .map(item => item.id) || [],
    [community?.channels],
  );
  ...
  useEffect(() => {
    const subscription = AppState.addEventListener('change', state => {
      appFocused.current = state === 'active';
    });
    ...
    async function synchronize() {
      try {
        ...
        if (activeConversation) {
          const direct = bootstrap.direct_messages.some(
            item => item.id === activeConversation,
          );
          const page = await client
            .listMessages(account.session_token, activeConversation, direct)
            .catch(() => undefined);
          if (page) {
        ...
        const stream = new RealtimeClient(
          account.instance_url,
          account.session_token,
          {
            ...
            onFrame: frame => {
              ...
              setCommunity(current =>
                current ? reduceRealtimeFrame(current, frame) : current,
              );
              const events =
                frame.type === 'events'
                  ? frame.events || []
              ...
              for (const event of events) {
                if (event.type !== 'message.created' || !event.payload)
                  continue;
                const state = communityRef.current;
                const message = event.payload as Message;
                if (!state) continue;
                const directMessage = state.direct_messages.find(
                  item => item.id === message.channel_id,
                );
                const channelItem = state.channels.find(
                  item => item.id === message.channel_id,
                );
                notifications
        ...
        const cached = await cache
          .load(account.instance_url, account.member.id)
          .catch(() => undefined);
        if (cached)
          setCommunity(
            current => current || communityStateFromBootstrap(cached),
          );
        setError(
          cached
            ? 'Showing cached Messages while reconnecting…'
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(
      () =>
        cache
          .save(account.instance_url, account.member.id, community)
          .catch(() => {}),
      1000,
    );
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      busy = true;
      try {
        const call = await client.currentCall(account.session_token);
        if (mounted) setCurrentCall(call);
      } catch {
      } finally {
        busy = false;
      }
    ...
    poll().catch(() => {});
    const timer = setInterval(() => poll().catch(() => {}), 1500);
    return () => {
      mounted = false;
      clearInterval(timer);
    };
        ...
        const states = await Promise.all(
          voiceChannelIDs.map(
            async id =>
              [
                id,
                await client.voiceRoomParticipants(account.session_token, id),
              ] as const,
          ),
        );
  ...
  async function openConversation(id: string, isDirect: boolean) {
    ...
    setHistory(value => ({
      ...value,
      [id]: {
        ...(value[id] || { hasMore: true, nextBefore: 0 }),
        loading: true,
      },
    }));
    try {
      ...
      setCommunity(value =>
        value
          ? { ...value, messages: { ...value.messages, [id]: page.messages } }
          : value,
      );
      setHistory(value => ({
        ...value,
    ...
    const current = community?.channel_states.find(
      item => item.channel_id === id,
    );
    if (!current || current.last_sequence <= current.read_sequence) return;
    try {
      const next = await client.updateReadPosition(
        account.session_token,
        id,
      ...
      setCommunity(value =>
        value
          ? {
              ...value,
              channel_states: [
                ...value.channel_states.filter(item => item.channel_id !== id),
                next,
              ],
            }
          : value,
    ...
    try {
      ...
      setCommunity(value => {
        ...
        return {
          ...
          messages: {
            ...value.messages,
            [activeID]: mergeMessagePage(
              value.messages[activeID] || [],
              page.messages,
            ),
          },
        };
  ...
  function trimHistory() {
    ...
    setHistory(value => ({
      ...value,
      [activeID]: {
        hasMore: trimmed[0].sequence > 1,
        nextBefore: trimmed[0].sequence,
        loading: false,
      },
    }));
    setCommunity(value =>
      value
        ? { ...value, messages: { ...value.messages, [activeID]: trimmed } }
        : value,
    );
  }
  ...
  async function send() {
    ...
    try {
      ...
      setCommunity(value =>
        value
          ? reduceRealtimeFrame(value, {
              type: editing ? 'message.edited' : 'message.created',
              cursor: value.cursor,
              channel_id: activeID,
              payload: message,
            })
  ...
  async function performMessageAction(
    action: 'reply' | 'edit' | 'delete' | 'pin',
    message: Message,
  ) {
        ...
        setCommunity(value =>
          value
            ? reduceRealtimeFrame(value, {
                ...
                cursor: value.cursor,
                channel_id: activeID,
                payload: { ...message, deleted: true },
              })
            : value,
        );
      } else {
        await client.setPinned(
          account.session_token,
          message.id,
          !message.pinned,
        );
        setCommunity(value =>
          value
            ? reduceRealtimeFrame(value, {
                ...
                cursor: value.cursor,
                channel_id: activeID,
                payload: { message_id: message.id, pinned: !message.pinned },
              })
            : value,
        );
      }
  ...
  async function openPins() {
    ...
    try {
      setPanelMessages(
        await client.pinnedMessages(account.session_token, activeID),
      );
    } catch (caught) {
      setError(
        caught instanceof Error
  ...
  async function search() {
    ...
    try {
      setSearchResults(
        (await client.searchMessages(account.session_token, searchQuery.trim()))
          .results,
      );
    } catch (caught) {
      setError(
        caught instanceof Error ? caught.message : 'Could not search Messages.',
      );
    } finally {
      setPanelBusy(false);
    }
  ...
  async function startDM(member: Member) {
    ...
    setError('');
    try {
      const item = await client.openDirectMessage(
        account.session_token,
        member.id,
      );
      setCommunity(value =>
        value
          ? {
              ...
              direct_messages: [
                item,
                ...value.direct_messages.filter(dm => dm.id !== item.id),
              ],
            }
          : value,
      );
      setMembersOpen(false);
      setProfileMember(undefined);
      await openConversation(item.id, true);
    } catch (caught) {
      setError(
        caught instanceof Error
          ? caught.message
          : 'Could not open the Direct Message.',
      );
    } finally {
      setPanelBusy(false);
    }
  ...
  async function changePresence(mode: 'available' | 'dnd') {
    try {
      await client.setPresenceMode(account.session_token, mode);
    } catch (caught) {
      setError(
        caught instanceof Error
          ? caught.message
    ...
    try {
      ...
      setCommunity(value =>
        value
          ? {
              ...value,
              direct_messages: value.direct_messages.map(item =>
                item.id === direct.id
                  ? { ...item, blocked_by_me: !item.blocked_by_me }
                  : item,
              ),
            }
          : value,
  ...
  async function startDirectCall() {
    if (!direct) return;
    try {
      setCurrentCall(await client.startCall(account.session_token, direct.id));
    } catch (caught) {
      setError(
  ...
  async function callAction(action: 'accept' | 'decline' | 'end') {
    ...
    try {
      ...
      setCurrentCall(
        next.state === 'ringing' || next.state === 'accepted'
          ? next
          : undefined,
      );
    } catch (caught) {
    ...
    try {
      ...
      setSelectedFiles(current =>
        [...current, ...chosen]
          .filter(
            (file, index, files) =>
              files.findIndex(item => item.uri === file.uri) === index,
          )
          .slice(0, 10),
      );
    } catch (caught) {
        ...
        {error ? (
          <>
            ...
            <TouchableOpacity onPress={onManage}>
              <Text style={{ color: palette.accent }}>Manage Instances</Text>
            </TouchableOpacity>
          </>
        ) : (
  ...
  const mediaSession = mediaRoom ? (
    <MediaRoomScreen
      account={account}
      key={mediaRoom.id}
      members={community.members}
      minimized={!mediaRoomOpen}
      name={mediaRoom.name}
      onExpand={() => setMediaRoomOpen(true)}
      onLeave={() => {
        setMediaRoom(undefined);
        setMediaRoomOpen(false);
      }}
      onMinimize={() => setMediaRoomOpen(false)}
  ...
  if (!activeID || (!channel && !direct)) {
    ...
    const openDM = (item: DirectMessage) => {
      setAllDMsOpen(false);
      openConversation(item.id, true);
    };
    const dmRow = (item: DirectMessage) => (
      <TouchableOpacity
        accessibilityLabel={`Direct Message with ${displayName(item)}`}
        key={item.id}
        onPress={() => openDM(item)}
        style={[
          styles.conversation,
          styles.compactConversation,
      ...
      >
        ...
        <Text
          numberOfLines={1}
          style={[styles.conversationName, { color: palette.text }]}
        >
          {displayName(item)}
        </Text>
      ...
      <>
        ...
        {!mediaRoomOpen ? (
          <View style={styles.fill}>
            ...
            >
              <View style={styles.communityHeaderIdentity}>
                <Text
                  ellipsizeMode="tail"
                  numberOfLines={1}
              ...
              {community.member.owner ? (
                <TouchableOpacity
                  accessibilityLabel="Moderation Reports"
                  onPress={() => setModerationOpen(true)}
                  style={[styles.iconButton, styles.communityHeaderAction]}
                >
                  <ShieldAlert color={palette.text} size={20} />
                </TouchableOpacity>
              ) : null}
              <TouchableOpacity
                accessibilityLabel="Notification Settings"
                onPress={() => setNotificationPanel('global')}
                style={[styles.iconButton, styles.communityHeaderAction]}
              >
              ...
              <TouchableOpacity
                ...
                onPress={() =>
                  changePresence(
                    community.presence[community.member.id] === 'dnd'
                      ? 'available'
                      : 'dnd',
                  )
                }
              ...
              <TouchableOpacity
                accessibilityLabel="Manage Instances"
            ...
            {currentCall ? (
              <CallBanner
                ...
                onAction={callAction}
                onOpen={() =>
                  openConversation(currentCall.direct_message_id, true)
                }
                palette={palette}
              />
            ) : null}
            <View style={styles.channelBrowser}>
              <View
                style={[
                  styles.dmBrowserSection,
                  { borderBottomColor: palette.border },
                ]}
              >
                <View style={styles.browserSectionHeader}>
                  <Text style={[styles.section, { color: palette.muted }]}>
                    DIRECT MESSAGES
                  </Text>
                  <TouchableOpacity
                    accessibilityLabel="Show all Direct Messages"
                    onPress={() => setAllDMsOpen(true)}
                  >
              ...
              <View style={styles.channelBrowserSection}>
                ...
                <FlatList
                  ...
                  ListEmptyComponent={
                    <Text style={{ color: palette.muted }}>
                      No Community channels are available.
                    </Text>
                  }
                  renderItem={({ item }) => {
                    const participants =
                      item.type === 'voice'
                        ? (voiceRooms[item.id]?.participants || []).filter(
                    ...
                    return (
                      <View>
                        <TouchableOpacity
                          onPress={() => {
                            if (item.type === 'voice') {
                              setMediaRoom({ id: item.id, name: item.name });
                              setMediaRoomOpen(true);
                            } else openConversation(item.id, false);
                          }}
                          style={[
                            styles.conversation,
                            { backgroundColor: palette.field },
                        ...
                        >
                          {item.type === 'voice' ? (
                            <Volume2 color={palette.muted} size={18} />
                          ) : (
                        ...
                        {participants.length ? (
                          <View style={styles.voiceMemberList}>
                            {participants.map(participant => {
                              const profile =
                                voiceRooms[item.id]?.members[
                                  participant.member_id
                                ];
                              const label = profile
                                ? memberName(profile)
                                : voiceRooms[item.id]?.names[
                                    participant.member_id
                                  ] || 'Member';
                              return (
                                <View
                                  key={participant.member_id}
                                  style={styles.voiceMemberRow}
                                >
                                  <MediaParticipantAvatar
                                    instanceURL={account.instance_url}
                                    label={label}
                                  ...
                                  participant.server_muted ? (
                                    <MicOff color={palette.muted} size={15} />
                                  ) : null}
            ...
            <Modal
              animationType="slide"
              onRequestClose={() => setAllDMsOpen(false)}
              visible={allDMsOpen}
            >
              <View
              ...
              >
                ...
                >
                  <Text
                    style={[styles.title, styles.grow, { color: palette.text }]}
                  >
                    Direct Messages
                  </Text>
                  <TouchableOpacity
                    accessibilityLabel="Close all Direct Messages"
                    onPress={() => setAllDMsOpen(false)}
                    style={styles.iconButton}
                  >
                    <Text style={[styles.headerIcon, { color: palette.text }]}>
                      ×
                    </Text>
                  </TouchableOpacity>
                </View>
                <FlatList
                  ...
                  data={community.direct_messages}
                  keyExtractor={item => item.id}
                  ListEmptyComponent={
                    <Text style={{ color: palette.muted }}>
                      No Direct Messages yet.
            ...
            <MemberProfile
              ...
              moderator={community.member.owner}
              ...
              onStartDM={startDM}
              palette={palette}
              self={profileMember?.id === community.member.id}
              token={account.session_token}
            />
            <ModerationPanel
              client={client}
            ...
            <NotificationSettingsPanel
              initial={community.notifications.community}
              mode={notificationPanel}
              onClose={() => setNotificationPanel('')}
  ...
  const typing = community.typing
    .filter(
      item =>
        item.channel_id === activeID &&
        item.member_id !== community.member.id &&
        ...
        >
          <View style={[styles.header, { borderBottomColor: palette.border }]}>
            ...
            {direct ? (
              <TouchableOpacity
                accessibilityLabel={
                  direct.blocked_by_me ? 'Unblock Member' : 'Block Member'
                }
                onPress={toggleBlock}
                style={styles.iconButton}
              >
                <Ban
                  color={direct.blocked_by_me ? '#ff6b6e' : palette.text}
                  size={20}
                />
              </TouchableOpacity>
            ) : null}
            {direct && !currentCall ? (
              <TouchableOpacity
                ...
                onPress={startDirectCall}
                style={styles.iconButton}
              >
                <Phone color={palette.text} size={20} />
              </TouchableOpacity>
            ) : null}
            {!direct ? (
              <TouchableOpacity
                ...
                style={styles.iconButton}
              >
                <Pin color={palette.text} size={20} />
              </TouchableOpacity>
            ) : null}
            <TouchableOpacity
            ...
            <TouchableOpacity
              ...
              onPress={() => setNotificationPanel('channel')}
              style={styles.iconButton}
            >
              <Bell color={palette.text} size={20} />
            </TouchableOpacity>
          ...
          {currentCall?.direct_message_id === activeID &&
          currentCall.state === 'accepted' ? (
            <View style={styles.directCallStage}>
              <MediaRoomScreen
                account={account}
                compact
                members={community.members}
                name={`Call with ${displayName(direct!)}`}
                onLeave={() => callAction('end')}
                onVoiceSettingsChange={onVoiceSettingsChange}
                palette={palette}
          ...
          <ConversationTimeline
            ...
            currentMemberID={community.member.id}
            key={activeID}
            loadingOlder={history[activeID]?.loading}
            messages={messages}
            onAtPresent={trimHistory}
            onLoadOlder={loadOlder}
            onMessageAction={setActionMessage}
            onReaction={toggleReaction}
            palette={palette}
          />
          {typing.length ? (
            <Text style={[styles.typing, { color: palette.muted }]}>
              {typingText(typing)}
            </Text>
          ) : null}
          {direct && (direct.blocked_by_me || direct.blocked_me) ? (
            <Text style={[styles.blockedNotice, { color: palette.muted }]}>
              Messages are disabled while either Member has blocked the other.
            </Text>
          ) : null}
          {error ? (
            <Text style={[styles.composerError, styles.errorColor]}>
              {error}
            </Text>
          ) : null}
          {selectedFiles.length ? (
            <ScrollView
              contentContainerStyle={styles.selectedFiles}
              horizontal
          ...
          {replying || editing ? (
            <View
              style={[styles.contextBanner, { backgroundColor: palette.field }]}
            >
              <Text
                numberOfLines={1}
                style={[styles.grow, { color: palette.text }]}
              >
                {editing
                  ? 'Editing your Message'
                  : `Replying to ${replying?.author_name}`}
              </Text>
              <TouchableOpacity
          ...
          <View style={[styles.composer, { borderTopColor: palette.border }]}>
            <TouchableOpacity
              ...
              disabled={sending || direct?.blocked_by_me || direct?.blocked_me}
              onPress={chooseAttachments}
              style={[
                styles.attach,
                { backgroundColor: palette.field },
                (direct?.blocked_by_me || direct?.blocked_me) &&
                  styles.disabled,
              ]}
            >
              <Text style={[styles.attachText, { color: palette.text }]}>
                +
              </Text>
            </TouchableOpacity>
            <TextInput
          ...
          {uploadStatus ? (
            <Text style={[styles.uploadStatus, { color: palette.muted }]}>
              {uploadStatus}
            </Text>
          ) : null}
          <MessageActions
            ...
            mine={Boolean(
              actionMessage && actionMessage.author_id === community.member.id,
            )}
            onAction={performMessageAction}
          ...
          <NotificationSettingsPanel
            initial={
              community.notifications.channels[activeID] || {
                level: 'default',
                muted: false,
              }
            }
...
function CallBanner({
  ...
  const incoming =
    call.state === 'ringing' && call.recipient_id === currentMemberID;
  return (
    <View style={[styles.callBanner, { backgroundColor: palette.field }]}>
      <TouchableOpacity disabled={!onOpen} onPress={onOpen} style={styles.grow}>
      ...
      {incoming ? (
        <>
          <TouchableOpacity
            ...
            style={[styles.callAction, styles.successBackground]}
          >
            <Text style={styles.whiteText}>Accept</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => onAction('decline')}
            style={[styles.callAction, styles.dangerBackground]}
          >
        ...
        >
          <Text style={styles.whiteText}>
            {call.state === 'accepted' ? 'End' : 'Cancel'}
          </Text>
        </TouchableOpacity>
      )}
    </View>
...
function MediaRoomScreen({
  ...
  const captureSettings = useRef(voiceSettings).current;
  const session = useRef<MediaSession | undefined>(undefined);
  const [status, setStatus] = useState<MediaStatus>('connecting');
  const [error, setError] = useState('');
  const [progress, setProgress] = useState('Requesting microphone permission…');
  ...
  const [local, setLocal] = useState<MediaStream>();
  ...
  useEffect(() => {
    ...
    const connection = new MediaSession({
      ...
      onStatus: (next, cause) => {
        ...
        if (next === 'recovering') setProgress('Reconnecting media…');
        if (next === 'failed' || next === 'idle') {
          setLocal(undefined);
          setScreen(undefined);
        }
      },
      ...
      onRemote: next => {
        ...
        setExpandedVideo(current =>
          current &&
          !current.self &&
          !next.some(item => item.stream === current.stream)
            ? undefined
            : current,
        );
      },
      ...
      onFrame: frame => {
        if (frame.type === 'soundboard-played' && frame.sound?.audio_url)
          setPlayingSound(
            attachmentURL(account.instance_url, frame.sound.audio_url),
          );
        if (frame.type === 'screen-rejected') {
          setSharing(false);
          setScreen(undefined);
          setError('Another Member is already sharing their screen.');
        }
      },
    });
    ...
    new AllChatClient(account.instance_url)
      .soundboard(account.session_token)
      .then(setSounds)
      .catch(() => setSounds([]));
    return () => {
      connection.stop();
  ...
  useEffect(() => {
    session.current
      ?.updateAudioSettings(voiceSettings)
      .catch(caught =>
        setError(
          caught instanceof Error
            ? caught.message
            : 'Could not apply audio processing settings.',
        ),
      );
  }, [voiceSettings]);
  useEffect(() => {
    const updateVisibility = (state = AppState.currentState) =>
      session.current?.setScreenVisible(!minimized && state === 'active');
    updateVisibility();
  ...
  async function toggleCamera() {
    try {
      const next = !camera;
      if (next && !(await requestMediaPermissions(true)))
      ...
      if (next) {
        setSharing(false);
        setScreen(undefined);
      } else setExpandedVideo(undefined);
      setLocal(session.current?.localStream());
      await updateCallForegroundService(next, false);
    } catch (caught) {
  ...
  function switchCamera() {
    try {
      session.current?.switchCamera();
    } catch (caught) {
      setError(
        caught instanceof Error ? caught.message : 'Could not switch camera.',
      );
    }
  }
  async function toggleScreen() {
    try {
      const next = !sharing;
      await session.current?.setScreenSharing(next);
      setSharing(next);
      if (next) {
        setCamera(false);
        setLocal(session.current?.localStream());
      } else setExpandedVideo(undefined);
      setScreen(session.current?.screenStream());
      await updateCallForegroundService(false, next);
    } catch (caught) {
      setError(
        caught instanceof Error ? caught.message : 'Screen sharing failed.',
      );
    }
  }
  function leave() {
  ...
  if (minimized)
    return (
      <View style={styles.voiceStatusStrip}>
        <TouchableOpacity
          accessibilityLabel="Return to Voice Room"
          onPress={onExpand}
          style={styles.voiceStatusSummary}
        >
          <Text numberOfLines={1} style={styles.voiceStatusTitle}>
            ● {name}
          </Text>
          <Text numberOfLines={1} style={styles.voiceStatusText}>
            {mediaStatusText(status)}
          </Text>
        </TouchableOpacity>
        <ScrollView
          contentContainerStyle={styles.voiceStatusActions}
          horizontal
          showsHorizontalScrollIndicator={false}
        >
          <TouchableOpacity
            ...
            style={[
              ...
              muted && styles.voiceStatusDanger,
            ]}
          >
            <Text style={styles.whiteText}>{muted ? 'Unmute' : 'Mute'}</Text>
          </TouchableOpacity>
          <TouchableOpacity
            accessibilityLabel={camera ? 'Stop camera' : 'Stop screen sharing'}
            disabled={!camera && !sharing}
            onPress={() => {
              if (camera) toggleCamera();
              else if (sharing) toggleScreen();
            }}
            style={[
              styles.voiceStatusButton,
              camera || sharing
                ? styles.voiceStatusDanger
                : styles.voiceStatusDisabled,
            ]}
          >
            <Text style={styles.whiteText}>
              {camera ? 'Stop video' : 'Stop share'}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            accessibilityLabel="Leave Voice Room"
            onPress={leave}
            style={[styles.voiceStatusButton, styles.voiceStatusDanger]}
          >
            <Text style={styles.whiteText}>Leave</Text>
          </TouchableOpacity>
          <TouchableOpacity
            accessibilityLabel="Go back to Voice Room"
            onPress={onExpand}
            style={styles.voiceStatusButton}
          >
  ...
  const knownMemberIDs = new Set(members.map(item => item.id));
  knownMemberIDs.add(account.member.id);
  const participantIDs = activeMediaParticipantIDs(
    account.member.id,
    participants,
  );
  const activeParticipantSet = new Set(participantIDs);
  const videos = new Map(
    remote
      .filter(
        item =>
          item.kind === 'video' &&
          knownMemberIDs.has(item.ownerID) &&
          activeParticipantSet.has(item.ownerID),
      )
      .map(item => [item.ownerID, item.stream]),
  );
  if (compact && !videos.size) {
    const unidentified = remote.find(
      item => item.kind === 'video' && !item.ownerID,
    );
    const otherParticipant = participantIDs.find(
      id => id !== account.member.id,
    );
    if (unidentified && otherParticipant)
      videos.set(otherParticipant, unidentified.stream);
  }
  const scrollingGrid = participantIDs.length > 4;
  const tiles = participantIDs.map(memberID => {
    const member =
      members.find(item => item.id === memberID) ||
      (memberID === account.member.id ? account.member : undefined);
    const video =
      memberID === account.member.id
        ? screen || (camera ? local : undefined)
        : videos.get(memberID);
    const label = member ? memberName(member) : 'Member';
    const volume = voiceSettings.memberVolumes[memberID] ?? 1;
    return (
      <MediaParticipantTile
        compact={compact}
        fillRow={participantIDs.length < 4}
        instanceURL={account.instance_url}
        key={memberID}
        member={member}
        memberID={memberID}
  ...
  const mediaGrid = scrollingGrid ? (
    <ScrollView
      contentContainerStyle={[
        styles.mediaGrid,
        styles.mediaGridScrollable,
        compact && styles.compactMediaGrid,
      ]}
    ...
    <View
      style={[
        styles.mediaGrid,
        participantIDs.length < 4 && styles.mediaGridVertical,
        compact && styles.compactMediaGrid,
      ]}
    >
      {tiles}
    ...
    <>
      {status !== 'connected' ? (
        <Text
          ...
          style={[styles.compactMediaStatus, styles.connected]}
        >
          {progress}
        </Text>
      ) : null}
  ...
  return (
    <View
      style={[
        styles.mediaRoom,
        compact && styles.compactMediaRoom,
        { backgroundColor: palette.background },
      ]}
    >
      {playingSound ? (
        <Video
          onEnd={() => setPlayingSound('')}
          paused={false}
          source={{
            uri: playingSound,
        ...
        <View style={[styles.header, { borderBottomColor: palette.border }]}>
          <View style={styles.grow}>
            <View style={styles.iconLabel}>
              <Volume2 color={palette.text} size={20} />
              <Text
                numberOfLines={1}
                style={[styles.title, styles.grow, { color: palette.text }]}
              >
          ...
          {onMinimize ? (
            <TouchableOpacity
              accessibilityLabel="Browse while connected"
              onPress={onMinimize}
              style={[styles.leaveButton, { backgroundColor: palette.field }]}
            >
              <Text style={{ color: palette.text }}>Browse</Text>
            </TouchableOpacity>
          ) : null}
          <TouchableOpacity
            accessibilityLabel="Leave Voice Room"
            onPress={leave}
            style={[styles.leaveButton, styles.dangerBackground]}
          >
            <Text style={styles.whiteText}>Leave</Text>
          </TouchableOpacity>
        </View>
      ...
      <ScrollView
        contentContainerStyle={styles.mediaControls}
        horizontal
        showsHorizontalScrollIndicator={false}
        style={[styles.mediaControlsScroll, { borderTopColor: palette.border }]}
      >
        <TouchableOpacity
          onPress={() => {
            const next = !muted;
        ...
        >
          <Text style={muted ? styles.whiteText : { color: palette.text }}>
            {muted ? 'Unmute' : 'Mute'}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          ...
          style={[
            ...
            camera
              ? styles.dangerBackground
              : { backgroundColor: palette.field },
            status !== 'connected' && styles.disabled,
          ]}
        >
          <Text style={camera ? styles.whiteText : { color: palette.text }}>
            {camera ? 'Stop video' : 'Camera'}
          </Text>
        </TouchableOpacity>
        {camera ? (
          <TouchableOpacity
            ...
            style={[styles.mediaControl, { backgroundColor: palette.field }]}
          >
            <Text style={{ color: palette.text }}>↻ Flip</Text>
          </TouchableOpacity>
        ) : null}
        <TouchableOpacity
          disabled={status !== 'connected'}
        ...
        >
          <Text style={sharing ? styles.whiteText : { color: palette.text }}>
        ...
        {sounds.length ? (
          <TouchableOpacity
            ...
            style={[styles.mediaControl, { backgroundColor: palette.field }]}
          >
            <Text style={{ color: palette.text }}>Sounds</Text>
          </TouchableOpacity>
        ) : null}
      </ScrollView>
      <SoundboardModal
        ...
        onPlay={sound => {
          session.current?.playSound(sound.id);
      ...
      >
        <View style={styles.mediaFullscreen}>
          {expandedVideo ? (
            <RTCView
              mirror={expandedVideo.self}
              objectFit="contain"
              streamURL={expandedVideo.stream.toURL()}
              style={styles.mediaFullscreenVideo}
            />
          ) : null}
          <Text style={styles.mediaFullscreenLabel}>
            {expandedVideo?.label}
          </Text>
          <TouchableOpacity
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        ...
        fillRow ? styles.mediaParticipantRow : styles.mediaParticipantCell,
        scrolling && styles.mediaParticipantScrollable,
    ...
    >
      {video ? (
        <RTCView
          ...
          style={styles.mediaParticipantVideo}
        />
      ) : (
        <MediaParticipantAvatar
      ...
      <Text
        numberOfLines={1}
        style={[styles.mediaParticipantName, { color: palette.text }]}
      >
        {label}
        {self ? ' (You)' : ''}
      </Text>
      <View style={styles.participantStateRow}>
        {!video ? <Mic color={palette.muted} size={12} /> : null}
        <Text style={[styles.mediaParticipantState, { color: palette.muted }]}>
          {video
            ? 'Video · tap to expand'
            : memberID
            ? 'Voice connected'
  ...
  useEffect(() => {
    ...
    if (member?.avatar_url)
      loadAuthenticatedImage(
        attachmentURL(instanceURL, member.avatar_url),
        token,
      )
        .then(value => {
...
function mediaStatusText(status: MediaStatus) {
  ...
  return (
    ...
    >
      <View style={styles.profileBackdrop}>
        <View
          style={[styles.profileCard, { backgroundColor: palette.background }]}
        >
          <Text style={[styles.profileName, { color: palette.text }]}>
            Soundboard
          </Text>
          <ScrollView contentContainerStyle={styles.soundList}>
            {sounds.map(sound => (
              <TouchableOpacity
                key={sound.id}
                onPress={() => onPlay(sound)}
                style={[styles.soundButton, { backgroundColor: palette.field }]}
              >
                {sound.emoji ? (
                  <Text style={styles.soundEmoji}>{sound.emoji}</Text>
                ) : (
                  <Radio color={palette.muted} size={22} />
                )}
                <Text
                  numberOfLines={1}
                  style={[styles.grow, { color: palette.text }]}
          ...
          <TouchableOpacity onPress={onClose} style={styles.profileAction}>
...
function FileTypeIcon({
  ...
  if (contentType.startsWith('audio/'))
    return <FileMusic color={color} size={size} />;
  if (contentType.startsWith('video/'))
    return <FileVideo color={color} size={size} />;
  ...
  return (
    ...
    >
      {image ? (
        ...
        <FileTypeIcon color={palette.muted} contentType={file.type || ''} />
      )}
      <Text
        numberOfLines={1}
      ...
      <TouchableOpacity
        accessibilityLabel={`Remove ${file.name || 'Attachment'}`}
        onPress={onRemove}
...
export function ConversationTimeline({
  ...
  return (
    <FlatList
      ...
      keyExtractor={item => item.id}
      ListFooterComponent={
        loadingOlder ? <ActivityIndicator color={palette.accent} /> : null
      }
      onEndReached={onLoadOlder}
      onEndReachedThreshold={0.25}
      onScroll={event => {
        if (event.nativeEvent.contentOffset.y < 80) onAtPresent?.();
      }}
      ...
      renderItem={({ item }) => (
        <MessageRow
          instanceURL={account.instance_url}
          message={item}
          mine={item.author_id === currentMemberID}
...
export function MessageRow({
  ...
  return (
    <TouchableOpacity
      activeOpacity={onLongPress ? 0.72 : 1}
      delayLongPress={350}
    ...
    >
      {message.reply ? (
        <View
          style={[styles.replyPreview, { borderLeftColor: palette.border }]}
        >
          <Text numberOfLines={1} style={{ color: palette.muted }}>
            ↳ {message.reply.author_name}:{' '}
            {message.reply.deleted ? 'deleted Message' : message.reply.body}
          </Text>
        </View>
      ) : null}
      <View style={styles.authorLine}>
        <Text
          style={[
            styles.author,
            { color: mine ? palette.accent : palette.text },
          ]}
        >
          {mine ? 'You' : message.author_name}
        </Text>
        <Text
          accessibilityLabel={`Sent ${new Date(
            message.created_at,
          ).toLocaleString()}`}
        ...
        {message.pinned ? (
          <Text style={{ color: palette.muted }}> ◆ Pinned</Text>
        ) : null}
      </View>
      {message.deleted ? (
        <Text style={[styles.messageBody, { color: palette.muted }]}>
          Message deleted
        </Text>
      ) : (
        <>
            ...
            <View style={styles.reactions}>
              {message.reactions.map(reaction => (
                <TouchableOpacity
                  ...
                  disabled={!onReaction}
                  key={reaction.emoji}
                  onPress={() => onReaction?.(message, reaction.emoji)}
                  style={[
                    styles.reaction,
                    {
                ...
                >
                  <Text
                    style={
                      reaction.me ? styles.whiteText : { color: palette.text }
                    }
                  >
  ...
  return (
    ...
    >
      ...
      <View style={styles.grow}>
        ...
        <Text style={{ color: palette.muted }}>
          {fileSize(attachment.size)} · {attachment.content_type || 'File'}
        </Text>
      </View>
    </View>
  );
...
function AuthenticatedImage({
  ...
  if (failed)
    return (
      ...
      >
        <ImageOff color={palette.muted} size={26} />
        <Text style={{ color: palette.muted }}>Image could not be loaded</Text>
      </View>
    );
  if (!dataURL)
    return (
      <View
        ...
        style={[styles.imagePlaceholder, { backgroundColor: palette.field }]}
      >
        <ActivityIndicator color={palette.accent} />
      </View>
    );
  ...
  const expand = () => {
    setOpen(true);
    if (!originalDataURL)
      loader(originalURL, token)
        .then(setOriginalDataURL)
        .catch(() => {});
  };
  return (
    <>
      ...
      >
        <Image
          ...
          resizeMode="contain"
          source={source}
          style={[styles.image, { backgroundColor: palette.field }]}
        />
      </TouchableOpacity>
      {open ? (
        <Modal
        ...
        >
          <View style={styles.viewer}>
            ...
            <Image
              accessibilityLabel={accessibilityLabel}
              resizeMode="contain"
              source={{ uri: originalDataURL || dataURL }}
...
function MessageLinkPreview({
  ...
  useEffect(() => {
    ...
    if (target)
      new AllChatClient(instanceURL)
        .linkPreview(token, target)
        .then(value => {
  ...
  return (
    ...
    >
      <View style={styles.linkContent}>
        ...
        <Text
          numberOfLines={2}
          style={[styles.linkTitle, { color: palette.accent }]}
        >
          {preview.title || preview.url}
        </Text>
        {preview.description ? (
          <Text numberOfLines={3} style={{ color: palette.text }}>
            {preview.description}
          </Text>
        ) : null}
      </View>
...
function PreviewImage({
  ...
  useEffect(() => {
    let mounted = true;
    const proxy = `${instanceURL}/api/v1/link-preview/image?url=${encodeURIComponent(
      imageURL,
    )}`;
    loadAuthenticatedImage(proxy, token)
      .then(value => {
        if (mounted) setSource(value);
      })
      .catch(() => {});
  ...
  return source ? (
    <Image source={{ uri: source }} style={styles.linkImage} />
  ) : null;
}
...
function MessageActions({
  ...
  message?: Message;
  mine: boolean;
  onAction(action: 'reply' | 'edit' | 'delete' | 'pin', message: Message): void;
  onClose(): void;
  onReaction(message: Message, emoji: string): void;
  palette: Palette;
}) {
      ...
      >
        <View style={[styles.sheet, { backgroundColor: palette.background }]}>
          ...
          <ActionButton
            ...
            onPress={() => onAction('pin', message)}
            palette={palette}
          />
          {mine && !message.deleted ? (
            <ActionButton
              label="Edit Message"
              onPress={() => onAction('edit', message)}
              palette={palette}
            />
          ) : null}
          {mine && !message.deleted ? (
            <ActionButton
              ...
              onPress={() => onAction('delete', message)}
              palette={palette}
            />
          ) : null}
        </View>
...
function ActionButton({
  ...
  return (
    <TouchableOpacity
      onPress={onPress}
      style={[styles.actionButton, { borderTopColor: palette.border }]}
    >
      <Text
        style={[
          styles.actionText,
          danger ? styles.dangerText : { color: palette.text },
        ]}
      >
        {label}
      </Text>
      ...
      <View style={[styles.panel, { backgroundColor: palette.background }]}>
        <View style={[styles.header, { borderBottomColor: palette.border }]}>
          <Text style={[styles.title, { color: palette.text }]}>
            {mode === 'pins' ? 'Pinned Messages' : 'Search'}
          </Text>
          <View style={styles.grow} />
          <TouchableOpacity
            ...
            style={styles.iconButton}
          >
            <Text style={[styles.contextClose, { color: palette.text }]}>
              ×
            </Text>
          </TouchableOpacity>
        </View>
        ...
        {busy ? (
          <ActivityIndicator color={palette.accent} style={styles.panelBusy} />
        ) : (
          <FlatList
...
function MembersPanel({
  ...
  return (
    <Modal animationType="slide" onRequestClose={onClose} visible={open}>
      <View style={[styles.panel, { backgroundColor: palette.background }]}>
        <View style={[styles.header, { borderBottomColor: palette.border }]}>
          <Text style={[styles.title, { color: palette.text }]}>Members</Text>
          <View style={styles.grow} />
          <TouchableOpacity
...
function MemberProfile({
  ...
  useEffect(() => {
    ...
    setProfileDisplayName(member?.display_name || '');
    setStatus('');
    setReporting(false);
  }, [member?.display_name, member?.id, member?.username]);
  if (!member) return null;
  async function report() {
    if (!reason.trim()) return;
    try {
      await client.reportMember(token, member!.id, reason.trim());
  ...
  async function chooseAvatar() {
    try {
      ...
      if (!file) return;
      setSaving(true);
      await client.updateAvatar(token, {
        uri: file.uri,
      ...
      onProfileUpdated({
        ...member!,
        avatar_url: `/api/v1/members/${member!.id}/avatar`,
      });
  ...
  async function removeAvatar() {
    try {
      setSaving(true);
      await client.removeAvatar(token);
      onProfileUpdated({ ...member!, avatar_url: undefined });
      setAvatarVersion(value => value + 1);
  ...
  async function moderate(action: ModerationAction) {
    if (reason.trim().length < 3) {
      setStatus('Provide a reason of at least three characters.');
      return;
    }
    try {
      setSaving(true);
      await client.moderateMember(
        token,
        member!.id,
        action,
        reason.trim(),
        action === 'timeout' || action === 'suspend' ? 60 : 0,
      );
        ...
        >
          ...
          {self ? (
            <>
              ...
              >
                <Text style={{ color: palette.text }}>Choose avatar</Text>
              </TouchableOpacity>
              {member.avatar_url ? (
                <TouchableOpacity
                  disabled={saving}
                  ...
                  style={styles.profileAction}
                >
                  <Text style={styles.dangerText}>Remove avatar</Text>
                </TouchableOpacity>
              ) : null}
            </>
          ) : moderating ? (
            <>
              ...
              <View style={styles.moderationActions}>
                {(
                  ['warn', 'timeout', 'kick', 'suspend'] as ModerationAction[]
                ).map(action => (
                  <TouchableOpacity
                    disabled={saving}
                    key={action}
                    onPress={() => moderate(action)}
                    style={[
              ...
              <TouchableOpacity
                onPress={() => setModerating(false)}
                style={styles.profileAction}
              >
                <Text style={{ color: palette.text }}>Cancel moderation</Text>
              </TouchableOpacity>
            </>
          ) : reporting ? (
            <>
              ...
              >
                <Text style={styles.whiteText}>Submit report</Text>
              </TouchableOpacity>
            </>
          ) : (
            <>
              <TouchableOpacity
                onPress={() => onStartDM(member)}
                style={[
                  styles.profilePrimary,
                  { backgroundColor: palette.accent },
                ]}
              ...
              <TouchableOpacity
                ...
                style={styles.profileAction}
              >
                <Text style={styles.dangerText}>Report Member</Text>
              </TouchableOpacity>
              {moderator && !member.owner ? (
                <TouchableOpacity
                  onPress={() => {
                    setModerating(true);
                    setReason('');
                  ...
                  style={styles.profileAction}
                >
                  <Text style={styles.dangerText}>Moderate Member</Text>
                </TouchableOpacity>
              ) : null}
            </>
          )}
...
function ModerationPanel({
  ...
  async function resolve(item: Report) {
    try {
      setBusy(true);
      const updated = await client.resolveReport(
        token,
        item.id,
        'Reviewed and resolved from the mobile moderation panel.',
      );
      setReports(current =>
    ...
    <Modal animationType="slide" onRequestClose={onClose} visible={open}>
      <View style={[styles.panel, { backgroundColor: palette.background }]}>
        <View style={[styles.header, { borderBottomColor: palette.border }]}>
          ...
          <View style={styles.grow} />
          <TouchableOpacity
            accessibilityLabel="Close Moderation"
            onPress={onClose}
            style={styles.iconButton}
          >
            <Text style={[styles.contextClose, { color: palette.text }]}>
        ...
        {busy && !reports.length ? (
          ...
          <FlatList
            ...
            keyExtractor={item => item.id}
            ListEmptyComponent={
              <Text style={{ color: palette.muted }}>No reports.</Text>
            }
            renderItem={({ item }) => (
              <View
                style={[styles.panelItem, { backgroundColor: palette.field }]}
              >
                <Text style={[styles.author, { color: palette.text }]}>
                  {item.status === 'open' ? 'Open report' : 'Resolved report'}
                </Text>
                <Text style={{ color: palette.text }}>{item.reason}</Text>
                <Text style={{ color: palette.muted }}>
                ...
                {item.status === 'open' ? (
                  ...
                  >
                    <Text style={styles.whiteText}>Mark resolved</Text>
                  </TouchableOpacity>
                ) : item.outcome ? (
                  <Text style={{ color: palette.muted }}>{item.outcome}</Text>
                ) : null}
              </View>
            )}
          />
...
function NotificationSettingsPanel({
  ...
  if (!mode) return null;
  const levels: NotificationSetting['level'][] =
    mode === 'channel'
      ? ['default', 'all_messages', 'mentions_only', 'nothing']
    ...
    <Modal animationType="fade" onRequestClose={onClose} transparent visible>
      <View style={styles.profileBackdrop}>
        ...
        >
          ...
          <Text style={[styles.settingLabel, { color: palette.muted }]}>
            Notification level
          </Text>
          <View style={styles.settingOptions}>
            {levels.map(level => (
              <TouchableOpacity
                key={level}
                onPress={() => setSetting(value => ({ ...value, level }))}
                style={[
                  styles.settingOption,
                  {
                    backgroundColor:
                      setting.level === level ? palette.accent : palette.field,
                  },
                ]}
              >
                <Text
          ...
          <TouchableOpacity
            onPress={() =>
              setSetting(value => ({ ...value, muted: !value.muted }))
            }
            style={[styles.settingToggle, { backgroundColor: palette.field }]}
          >
            <Text style={{ color: palette.text }}>
              {setting.muted ? 'Muted' : 'Not muted'}
            </Text>
          </TouchableOpacity>
          {mode === 'global' ? (
            <TouchableOpacity
            ...
            >
              <Text style={{ color: palette.text }}>
                {setting.sound_enabled === false ? 'Sound off' : 'Sound on'}
              </Text>
            </TouchableOpacity>
          ) : null}
          {status ? (
          ...
          <TouchableOpacity
            disabled={saving}
            onPress={save}
            style={[styles.profilePrimary, { backgroundColor: palette.accent }]}
          >
            <Text style={styles.whiteText}>{saving ? 'Saving…' : 'Save'}</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={onClose} style={styles.profileAction}>
            <Text style={{ color: palette.text }}>Close</Text>
          </TouchableOpacity>
...
function notificationLevelName(level: NotificationSetting['level']) {
...
function InlineMedia({
  ...
  return (
    ...
    >
      {video ? (
        <Play color={palette.muted} size={26} />
      ) : (
        <FileMusic color={palette.muted} size={26} />
      )}
      <View style={styles.grow}>
        <Text
          numberOfLines={1}
        ...
        <Text style={{ color: palette.muted }}>
          Tap to play · {fileSize(attachment.size)}
        </Text>
      </View>
    </TouchableOpacity>
  );
...
function attachmentURL(instanceURL: string, value: string) {
...
function displayName(dm: DirectMessage) {
  return dm.other.display_name || dm.other.username;
}
function unreadFor(state: CommunityState, id: string) {
  return state.channel_states.find(item => item.channel_id === id)?.unread || 0;
}
...
const styles = StyleSheet.create({
  iconLabel: { alignItems: 'center', flexDirection: 'row', gap: 8 },
  fill: { flex: 1 },
  grow: { flex: 1 },
  center: {
    alignItems: 'center',
  ...
  error: { color: '#ed4245', fontSize: 15, textAlign: 'center' },
  errorColor: { color: '#ed4245' },
  connected: { color: '#3ba55d' },
  header: {
    alignItems: 'center',
  ...
  headerIcon: { fontSize: 25 },
  communityHeader: { paddingHorizontal: 8 },
  communityHeaderIdentity: { flex: 1, minWidth: 0 },
  communityHeaderAction: { height: 40, width: 40 },
  presenceButton: { fontSize: 20 },
  presenceDot: { fontSize: 18 },
  presenceOnline: { color: '#3ba55d' },
  presenceDND: { color: '#ed4245' },
  presenceIdle: { color: '#faa61a' },
  presenceOffline: { color: '#747f8d' },
  conversationList: { gap: 8, padding: 16 },
  section: {
    fontSize: 12,
    fontWeight: '800',
    letterSpacing: 1.2,
  ...
  browserAvatarImage: { borderRadius: 15, height: 30, width: 30 },
  voiceMemberList: { gap: 3, paddingLeft: 26, paddingTop: 3 },
  voiceMemberRow: {
    alignItems: 'center',
    flexDirection: 'row',
    gap: 7,
  ...
  time: { fontSize: 11, marginLeft: 8, marginRight: 4 },
  image: {
    borderRadius: 8,
    height: 240,
    marginTop: 8,
  ...
  viewerCloseText: { color: '#ffffff', fontSize: 34, lineHeight: 38 },
  video: {
    borderRadius: 8,
    height: 240,
    marginTop: 8,
  ...
  attachmentIcon: { fontSize: 26 },
  attachmentName: { fontSize: 14, fontWeight: '700' },
  typing: { fontSize: 12, minHeight: 20, paddingHorizontal: 14 },
  composerError: { fontSize: 12, paddingHorizontal: 14, paddingBottom: 4 },
  selectedFiles: { gap: 8, paddingHorizontal: 10, paddingVertical: 8 },
  selectedFile: {
    alignItems: 'center',
    borderRadius: 8,
    borderWidth: 1,
  ...
  selectedThumbnail: { height: 52, marginRight: 8, width: 52 },
  selectedIcon: { fontSize: 24, marginHorizontal: 10 },
  ...
  selectedRemoveText: { color: '#ed4245', fontSize: 24 },
  composer: {
    alignItems: 'flex-end',
  ...
  attachText: { fontSize: 27, lineHeight: 30 },
  composerInput: {
    borderRadius: 20,
    flex: 1,
    fontSize: 16,
  ...
  uploadStatus: { fontSize: 12, paddingBottom: 6, paddingHorizontal: 14 },
  ...
  contextClose: { fontSize: 28, paddingHorizontal: 8 },
  sheetBackdrop: {
  ...
  quickReactions: { flexDirection: 'row', gap: 8, marginBottom: 12 },
  quickReaction: {
    alignItems: 'center',
    borderRadius: 22,
    height: 44,
  ...
  profileAction: { alignItems: 'center', padding: 13 },
  reportInput: {
    borderRadius: 10,
    marginTop: 16,
    minHeight: 100,
  ...
  linkContent: { flex: 1, gap: 4, justifyContent: 'center', padding: 10 },
  linkTitle: { fontSize: 15, fontWeight: '800' },
  linkImage: { height: 112, width: 112 },
  moderationActions: { gap: 8, marginTop: 12 },
  moderationAction: { alignItems: 'center', borderRadius: 8, padding: 12 },
  panelStatus: { paddingHorizontal: 12, paddingTop: 12 },
  resolveButton: {
    alignItems: 'center',
    borderRadius: 8,
    marginTop: 10,
  ...
  mediaGridVertical: { flexDirection: 'column', flexWrap: 'nowrap' },
  mediaGridScrollable: { flex: 0 },
  mediaParticipant: {
    alignItems: 'center',
    borderRadius: 12,
    gap: 7,
  ...
  mediaParticipantRow: { flex: 1, width: '100%' },
  mediaParticipantCell: { flexGrow: 1, height: '48%', width: '47%' },
  mediaParticipantScrollable: { height: 200 },
  mediaParticipantVideo: {
    backgroundColor: '#000',
    borderRadius: 9,
    flex: 1,
    width: '100%',
  ...
  mediaError: { color: '#ed4245', paddingHorizontal: 14, textAlign: 'center' },
  mediaFullscreen: {
    backgroundColor: '#000',
    flex: 1,
    justifyContent: 'center',
  },
  mediaFullscreenVideo: { height: '100%', width: '100%' },
  ...
  voiceStatusSummary: { flex: 1, minWidth: 100, paddingRight: 8 },
  voiceStatusTitle: { color: '#fff', fontSize: 14, fontWeight: '800' },
  voiceStatusText: { color: '#d9f7e3', fontSize: 11 },
  voiceStatusActions: {
    alignItems: 'center',
    flexDirection: 'row',
    gap: 6,
  ...
  voiceStatusDanger: { backgroundColor: '#c83c3e' },
  voiceStatusDisabled: { opacity: 0.42 },
  callBanner: {
    alignItems: 'center',
    borderRadius: 8,
    flexDirection: 'row',
  ...
  callAction: { borderRadius: 8, paddingHorizontal: 12, paddingVertical: 9 },
  directCallStage: { height: 330 },
  ...
  settingOptions: { gap: 8, marginTop: 8 },
  settingOption: { borderRadius: 8, padding: 11 },
  settingToggle: { borderRadius: 8, marginTop: 8, padding: 12 },
  soundList: { gap: 8, marginTop: 14, maxHeight: 360 },
  soundButton: {
    alignItems: 'center',
    borderRadius: 8,
    flexDirection: 'row',
