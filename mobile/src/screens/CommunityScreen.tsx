...
function MediaRoomScreen({account, compact = false, members, minimized = false, name, onExpand, onLeave, onMinimize, palette, roomID}: {account: InstanceAccount; compact?: boolean; members: Member[]; minimized?: boolean; name: string; onExpand?(): void; onLeave(): void; onMinimize?(): void; palette: Palette; roomID: string}) {
  ...
  const mediaGrid = scrollingGrid ? <ScrollView contentContainerStyle={[styles.mediaGrid, styles.mediaGridScrollable, compact && styles.compactMediaGrid]} style={styles.mediaGridScroll}>{tiles}</ScrollView> : <View style={[styles.mediaGrid, compact && styles.compactMediaGrid]}>{tiles}</View>;
  const grid = <>{status !== 'connected' ? <Text accessibilityLiveRegion="polite" style={[styles.compactMediaStatus, styles.connected]}>{progress}</Text> : null}{mediaGrid}{camera ? <TouchableOpacity accessibilityLabel="Switch front and rear camera" onPress={switchCamera} style={[styles.cameraSwitch, {backgroundColor: palette.field}]}><Text style={{color: palette.text}}>↻ Flip</Text></TouchableOpacity> : null}</>;
...
const styles = StyleSheet.create({
  ...
  cameraSwitch: {alignItems: 'center', borderRadius: 18, bottom: 76, paddingHorizontal: 14, paddingVertical: 9, position: 'absolute', right: 14, zIndex: 2},
