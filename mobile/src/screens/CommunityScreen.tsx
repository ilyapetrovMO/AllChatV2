...
function MediaRoomScreen({account, compact = false, name, onLeave, palette, roomID}: {account: InstanceAccount; compact?: boolean; name: string; onLeave(): void; palette: Palette; roomID: string}) {
  ...
  useEffect(() => {
    const connection = new MediaSession({instanceURL: account.instance_url, token: account.session_token, roomID, onStatus: (next, cause) => { setStatus(next); setError(cause?.message || ''); if (next === 'failed' || next === 'idle') { setLocal(undefined); setScreen(undefined); } }, onRemote: setRemote, onFrame: frame => { if (frame.type === 'soundboard-played' && frame.sound?.audio_url) setPlayingSound(attachmentURL(account.instance_url, frame.sound.audio_url)); if (frame.type === 'screen-rejected') { setSharing(false); setScreen(undefined); setError('Another Member is already sharing their screen.'); } }});
