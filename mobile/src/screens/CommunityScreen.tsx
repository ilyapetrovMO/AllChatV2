...
function MediaRoomScreen({account, compact = false, members, name, onLeave, palette, roomID}: {account: InstanceAccount; compact?: boolean; members: Member[]; name: string; onLeave(): void; palette: Palette; roomID: string}) {
  ...
  const grid = scrollingGrid ? <ScrollView contentContainerStyle={[styles.mediaGrid, styles.mediaGridScrollable, compact && styles.compactMediaGrid]} style={styles.mediaGridScroll}>{tiles}</ScrollView> : <View style={[styles.mediaGrid, compact && styles.compactMediaGrid]}>{tiles}</View>;
