...
import type {DirectCall, LinkPreview, Member, ModerationAction, Report, SoundboardSound} from '../client/AllChatClient';
...
export function CommunityScreen({account, palette, onManage}: {account: InstanceAccount; palette: Palette; onManage(): void}): React.JSX.Element {
  ...
  if (mediaRoom) return <MediaRoomScreen account={account} members={community.members} name={mediaRoom.name} onLeave={() => setMediaRoom(undefined)} palette={palette} roomID={mediaRoom.id} />;
...
function MediaRoomScreen({account, compact = false, members, name, onLeave, palette, roomID}: {account: InstanceAccount; compact?: boolean; members: Member[]; name: string; onLeave(): void; palette: Palette; roomID: string}) {
  ...
  useEffect(() => {
    const connection = new MediaSession({instanceURL: account.instance_url, token: account.session_token, roomID, onStatus: (next, cause) => { setStatus(next); setError(cause?.message || ''); if (next === 'failed' || next === 'idle') { setLocal(undefined); setScreen(undefined); } }, onRemote: setRemote, onParticipants: setParticipants, onFrame: frame => { if (frame.type === 'soundboard-played' && frame.sound?.audio_url) setPlayingSound(attachmentURL(account.instance_url, frame.sound.audio_url)); if (frame.type === 'screen-rejected') { setSharing(false); setScreen(undefined); setError('Another Member is already sharing their screen.'); } }});
  ...
  async function toggleCamera() { try { if (!camera && !await requestMediaPermissions(true)) throw new Error('Camera permission is required.'); await session.current?.setCamera(!camera); const next = !camera; setCamera(next); setLocal(session.current?.localStream()); await updateCallForegroundService(next, sharing); } catch (caught) { setError(caught instanceof Error ? caught.message : 'Camera failed.'); } }
  async function toggleScreen() { try { await session.current?.setScreenSharing(!sharing); const next = !sharing; setSharing(next); setScreen(session.current?.screenStream()); await updateCallForegroundService(camera, next); } catch (caught) { setError(caught instanceof Error ? caught.message : 'Screen sharing failed.'); } }
...
function MediaParticipantAvatar({instanceURL, label, member, palette, token}: {instanceURL: string; label: string; member?: Member; palette: Palette; token: string}) {
  ...
  return source ? <Image source={{uri: source}} style={styles.mediaParticipantAvatarImage} /> : <Text style={[styles.mediaParticipantAvatar, {backgroundColor: palette.border, color: palette.text}]}>{label.slice(0, 1).toUpperCase()}</Text>;
}
...
const styles = StyleSheet.create({
  ...
  mediaRoom: {flex: 1, minWidth: 0}, leaveButton: {borderRadius: 8, paddingHorizontal: 14, paddingVertical: 10}, mediaGrid: {alignContent: 'center', alignItems: 'stretch', flexDirection: 'row', flexGrow: 1, flexWrap: 'wrap', gap: 12, justifyContent: 'center', padding: 12}, mediaParticipant: {alignItems: 'center', borderRadius: 12, gap: 7, height: 220, justifyContent: 'center', minWidth: 140, overflow: 'hidden', padding: 10, width: '47%'}, mediaParticipantVideo: {backgroundColor: '#000', borderRadius: 9, flex: 1, width: '100%'}, mediaParticipantAvatar: {borderRadius: 48, fontSize: 36, fontWeight: '800', height: 96, lineHeight: 96, overflow: 'hidden', textAlign: 'center', width: 96}, mediaParticipantAvatarImage: {borderRadius: 48, height: 96, width: 96}, mediaParticipantName: {fontSize: 15, fontWeight: '800', maxWidth: '100%'}, mediaParticipantState: {fontSize: 11}, mediaControlsScroll: {borderTopWidth: StyleSheet.hairlineWidth, flexGrow: 0}, mediaControls: {alignItems: 'center', flexDirection: 'row', gap: 8, paddingHorizontal: 12, paddingVertical: 10}, mediaControl: {alignItems: 'center', borderRadius: 22, flexShrink: 0, minWidth: 80, paddingHorizontal: 13, paddingVertical: 11}, mediaError: {color: '#ed4245', paddingHorizontal: 14, textAlign: 'center'},
  callBanner: {alignItems: 'center', borderRadius: 8, flexDirection: 'row', gap: 8, margin: 8, padding: 10}, callAction: {borderRadius: 8, paddingHorizontal: 12, paddingVertical: 9}, directCallStage: {height: 330}, compactMediaRoom: {minHeight: 0}, compactMediaStatus: {fontSize: 12, paddingHorizontal: 12, paddingTop: 6}, compactMediaGrid: {alignContent: 'flex-start', flexGrow: 0, minHeight: 190, padding: 8}, compactMediaParticipant: {height: 165, minWidth: 130, width: '47%'},
