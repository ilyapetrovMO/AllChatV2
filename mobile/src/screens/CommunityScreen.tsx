...
import {DEFAULT_VOICE_VIDEO_SETTINGS, type VoiceVideoSettings} from '../media/VoiceVideoSettings';
...
export function activeMediaParticipantIDs(selfID: string, participants: MediaParticipant[]) {
  return [...new Set([selfID, ...participants.filter(item => item.connected !== false).map(item => item.member_id)])];
}

export function CommunityScreen({account, palette, onManage, onVoiceSettingsChange = () => {}, voiceSettings = DEFAULT_VOICE_VIDEO_SETTINGS}: {account: InstanceAccount; palette: Palette; onManage(): void; onVoiceSettingsChange?(settings: VoiceVideoSettings): void; voiceSettings?: VoiceVideoSettings}): React.JSX.Element {
  ...
  return (<>
    ...
    {!mediaRoomOpen ? <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.fill}>
      ...
      {currentCall?.direct_message_id === activeID && currentCall.state === 'accepted' ? <View style={styles.directCallStage}><MediaRoomScreen account={account} compact members={community.members} name={`Call with ${displayName(direct!)}`} onLeave={() => callAction('end')} onVoiceSettingsChange={onVoiceSettingsChange} palette={palette} roomID={currentCall.id} voiceSettings={voiceSettings} /></View> : null}
...
function MediaRoomScreen({account, compact = false, members, minimized = false, name, onExpand, onLeave, onMinimize, onVoiceSettingsChange, palette, roomID, voiceSettings}: {account: InstanceAccount; compact?: boolean; members: Member[]; minimized?: boolean; name: string; onExpand?(): void; onLeave(): void; onMinimize?(): void; onVoiceSettingsChange(settings: VoiceVideoSettings): void; palette: Palette; roomID: string; voiceSettings: VoiceVideoSettings}) {
  ...
  const participantIDs = activeMediaParticipantIDs(account.member.id, participants);
  const activeParticipantSet = new Set(participantIDs);
  const videos = new Map(remote.filter(item => item.kind === 'video' && knownMemberIDs.has(item.ownerID) && activeParticipantSet.has(item.ownerID)).map(item => [item.ownerID, item.stream]));
  ...
  const tiles = participantIDs.map(memberID => { const member = members.find(item => item.id === memberID) || (memberID === account.member.id ? account.member : undefined); const video = memberID === account.member.id ? (screen || (camera ? local : undefined)) : videos.get(memberID); const label = member ? memberName(member) : 'Member'; const volume = voiceSettings.memberVolumes[memberID] ?? 1; return <MediaParticipantTile compact={compact} fillRow={participantIDs.length < 4} instanceURL={account.instance_url} key={memberID} member={member} memberID={memberID} onOpenVideo={video ? () => setExpandedVideo({label, self: memberID === account.member.id, stream: video}) : undefined} onVolumeChange={memberID === account.member.id ? undefined : () => { const levels = [1, .75, .5, .25, 0], next = levels[(levels.indexOf(volume) + 1) % levels.length], settings = {...voiceSettings, memberVolumes: {...voiceSettings.memberVolumes, [memberID]: next}}; session.current?.updateAudioSettings(settings); onVoiceSettingsChange(settings); }} palette={palette} scrolling={scrollingGrid} self={memberID === account.member.id} token={account.session_token} video={video} volume={volume} />; });
...
function MediaParticipantTile({compact, fillRow, instanceURL, member, memberID, onOpenVideo, onVolumeChange, palette, scrolling, self, token, video, volume = 1}: {compact: boolean; fillRow: boolean; instanceURL: string; member?: Member; memberID: string; onOpenVideo?(): void; onVolumeChange?(): void; palette: Palette; scrolling: boolean; self: boolean; token: string; video?: MediaStream; volume?: number}) {
  const label = member ? memberName(member) : 'Member';
  return <TouchableOpacity accessibilityHint={onVolumeChange ? 'Long press to lower this Member volume' : undefined} accessibilityLabel={`${label}${self ? ', you' : ''}${video ? ', open video fullscreen' : ''}`} disabled={!video && !onVolumeChange} onLongPress={onVolumeChange} onPress={onOpenVideo} style={[styles.mediaParticipant, fillRow ? styles.mediaParticipantRow : styles.mediaParticipantCell, scrolling && styles.mediaParticipantScrollable, compact && styles.compactMediaParticipant, {backgroundColor: palette.field}]}>{video ? <RTCView mirror={self} objectFit="cover" streamURL={video.toURL()} style={styles.mediaParticipantVideo} /> : <MediaParticipantAvatar instanceURL={instanceURL} label={label} member={member} palette={palette} token={token} />}<Text numberOfLines={1} style={[styles.mediaParticipantName, {color: palette.text}]}>{label}{self ? ' (You)' : ''}</Text><Text style={[styles.mediaParticipantState, {color: palette.muted}]}>{video ? 'Video · tap to expand' : `🎙 ${memberID ? 'Voice connected' : 'Connecting'}`}{onVolumeChange ? ` · ${Math.round(volume * 100)}% (hold)` : ''}</Text></TouchableOpacity>;
}
...
export function MessageRow({imageLoader, instanceURL, message, mine, onLongPress, onReaction, palette, token}: {imageLoader?: ImageLoader; instanceURL: string; message: Message; mine: boolean; onLongPress?(message: Message): void; onReaction?(message: Message, emoji: string): void; palette: Palette; token: string}) {
  return <TouchableOpacity activeOpacity={onLongPress ? 0.72 : 1} delayLongPress={350} disabled={!onLongPress} onLongPress={() => onLongPress?.(message)} style={styles.message}>
    ...
    <View style={styles.authorLine}><Text style={[styles.author, {color: mine ? palette.accent : palette.text}]}>{mine ? 'You' : message.author_name}</Text><Text accessibilityLabel={`Sent ${new Date(message.created_at).toLocaleString()}`} style={[styles.time, {color: palette.muted}]}>{formatMessageTime(message.created_at)}{message.edited_at ? ' · edited' : ''}</Text>{message.pinned ? <Text style={{color: palette.muted}}> ◆ Pinned</Text> : null}</View>
