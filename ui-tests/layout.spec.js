...
test('community search stays in the top-right header across web screens', async ({page}) => {
  ...
  await expect(page.locator('[data-app-overlay] .content-header [role="search"]')).toHaveCount(1);
  await page.locator('[data-app-overlay] a[href="/sessions"]').click();
  await expect(page.locator('[data-app-overlay] .content-header [role="search"]')).toHaveCount(1);
  await expect(page.locator('[data-app-overlay] .content > form[action="/search"]')).toHaveCount(0);
  await page.locator('[data-app-overlay] .content-header [role="search"] input').fill('Welcome');
  await page.locator('[data-app-overlay] .content-header [role="search"] input').press('Enter');
...
test('owner-configured Markdown is rendered on Community Home', async ({page}) => {
  ...
  await expect(page.locator('.community-markdown table tbody td')).toHaveText(['Guide', 'Yes']);
  await expect(page.locator('.community-markdown li input[type="checkbox"]')).toBeChecked();
  await expect(page.locator('.community-markdown pre code.language-json')).toBeVisible();
  await expect(page.locator('.community-markdown pre .syntax-key').first()).toContainText('community');
  await expect(page.locator('.community-markdown pre .syntax-string')).toContainText('AllChat');
  await expect(page.locator('.community-markdown pre .syntax-literal')).toHaveText('true');
  await page.goto('/admin/settings');
